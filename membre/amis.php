...
$query=$bdd->prepare('SELECT membre_id, COUNT(*) AS nbr FROM
membres
...
if ($deja_ami != 0)
{
echo '<p>Ce membre fait déjà parti de vos amis ou a déjà
...
$i++;
	...
	if ($i == 0)
	{
		...
		m='.$data['membre_id'].'">'.stripslashes(htmlspecialchars($pseudo_d)).'</a>
...
if (empty($add))
{
	...
	<td>'.$data['ami_date'].'</td><td><a href="./amis.php?action=check&amp;add=ok&amp;m='.$data['ami_from'].'">Accepter</<a href="./amis.php?action=delete&amp;m='.$data['ami_from'].'">Refuser</a></td></tr>';
	}
...
if (!isset($_GET['ok']))
{
echo '<p>Etes vous certain de vouloir supprimer ce membre ?
<br />
<a href="./amis.php?
action=delete&amp;ok=ok&amp;m='.$membre.'">oui</a> - <a
href="./amis.php">non</a></p>';
}
else
{
...
<li><a href="./amis.php?action=check">Voir les demandes d\'ajout ('.$demande_ami.')';
