...
DATE_FORMAT(post_time ,\'%d/%m/%Y à %H:%i:%s\') AS post_time,
membre_id, membre_pseudo, DATE_FORMAT(membre_inscrit, \'%d/%m/%Y à %H:%i:%s\') AS membre_inscrit, membre_avatar,
...
while ($data = $query->fetch())
{
  ...
  {
  echo'<td id=p_'.$data['post_id'].'>Posté le '.$data['post_time'].'
  ...
  {
  echo'<td>Posté le '.$data['post_time'].'
  </td></tr>';
  }
